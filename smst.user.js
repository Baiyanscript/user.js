// ==UserScript==
// @name           SwordMasters.io 低風險殺人乘數 (防 ban 加強版)
// @namespace      http://tampermonkey.net/
// @version        1.1
// @description    低調重複發送 checkDamage，加入隨機延遲、機率丟包、上限保護
// @author         Baiyan
...
(function() {
    ...
    // ===================== 可調整參數 =====================
    const MAX_MULTIPLIER = 3000;          // 最高只到 x3000，超過自動限縮（防太誇張被 ban）
    const BASE_EXTRA_CHANCE = 0.75;     // 基礎額外發送機率 (75% → 平均 x1.75+)
    const MAX_EXTRA_PER_HIT = 8;        // 單次攻擊最多額外發幾個（硬上限）
    const MIN_DELAY_MS = 40;            // 最短延遲
    const MAX_DELAY_MS = 180;           // 最長延遲（模擬人類不規則）
    const GLOBAL_MSG_LIMIT_PER_SEC = 14; // 每秒最多發送 checkDamage 總數（包含原本的）
    // ======================================================
    ...
    console.log('[LowRisk Kill Multiplier] 已載入 - 低調模式 v1.1');
})();
